...
- Folder context menus include upload, so imported files or directories can target that folder directly. Blank-space uploads still target the selected folder or root through the backend import bridge.
- Add and context menus close when clicking outside the floating menu area.
