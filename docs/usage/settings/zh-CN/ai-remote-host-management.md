...
实际操作流程见[主机 Agent](../../best-practices/zh-CN/03-host-agent.md)。
...
- 命令安全：通过可视化表单维护结构化命令的黑名单、白名单、危险命令和风险审批规则。人工终端输入和粘贴不受该策略影响。高级 JSON 字段说明见[命令安全设置](command-security.md)。
