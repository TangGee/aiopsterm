...
The AI panel renders that backend-owned message only as a command card, not as duplicate free-form text. The card has a `Command` header, target host metadata when available, line count, copy action, executable command text, a header review/edit icon that opens a large review/edit dialog, and Reject / Execute controls. The action row wraps in narrow sidebars and keeps full button meanings in hover labels when text is truncated. The review dialog opens the generated command in a near-full-window monospaced editor before execution; saving updates the current command card's structured command data and syncs the conversation snapshot, and Save and Run executes that edited command through the same terminal path. Failed, already-sent, or rejected command cards can be reopened and edited again; saving a modified command clears the stale execution/rejection state so the card can be executed again. Only a currently running command is locked read-only. Read-only diagnostic commands also show `查询类自动执行`; destructive, write, restart, install, delete, interactive, or uncertain commands stay approval-gated and do not show that read-only shortcut. Copy uses the structured command text instead of the whole assistant message. Execute and read-only auto-execute send the command through the existing terminal command path and require an active backend terminal session; without one, aiopsterm shows the normal terminal-unavailable notice, marks the card as failed, and does not report a fake command result. Successful writes are accepted only after the terminal bridge confirms the exact live session write. Reject persists `action: "rejected"` on the message and disables terminal actions until the command is reopened and saved with an audited edit.
...
The right AI sidebar now keeps a External reference-style open conversation tab strip inside the top AI header. The tab strip is the set of currently opened AI conversations, not the full history list: the active backend-selected conversation is opened automatically, New Chat keeps the previous tab visible while adding the backend-created empty conversation, and restoring a conversation from History opens it as another tab. Tabs shrink and scroll inside the header, while the right side keeps only New Chat and More visible. More opens History, Search, and Export without allowing many tabs to push the fixed header actions out of view. Closing a tab only removes it from the visible tab strip and switches to the nearest open conversation; it does not delete the persisted history row. Default `新会话` / `Untitled chat` titles are renamed from the first sent prompt only through the persisted conversation snapshot, so the header tabs, History rows, and Agents sidebar all update from the same backend-owned metadata. Deleting history remains a separate History menu action backed by `deleteChatConversation()`.
