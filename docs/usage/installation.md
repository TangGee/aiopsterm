...
Local terminals created by aiopsterm expose `aio`, `aictl`, `aiopsterm-control`, and `aiossh` on PATH. `aio` is the preferred short command for the control helper. `aiossh <managed-host>` is the short SSH entry for hosts already saved in aiopsterm. These shims internally use aiopsterm's packaged runtime instead of relying on a system `node` binary.
