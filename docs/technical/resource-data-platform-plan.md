...
| Windows | Configured with local native-build evidence; release support not yet certified | `electron-builder.yml` has a `win` target, `package.json` exposes `build:win`, `build:win:dir`, `build:windows:one-click`, and `package:build -- windows`, and the supplied Windows patch records a successful native build. The one-click script can prepare and verify a local Windows runner without a remote repository. Runtime paths use named pipes, Windows-native process invocation, and explicit SQLite/socket cleanup. | Re-run the current integrated tree on Windows with the full unit, E2E, installer, and `package:verify -- windows` gates before declaring release support. |
...
| Secrets and keys | asset secrets, database credentials, credential key files | `assetsCredentialRuntime`, `databaseCredentialStorage` | Keep ciphertext and presence flags during startup/list operations, and decrypt only for an explicit connection or credential edit. Windows/Linux use Electron `safeStorage` when available; unsigned macOS builds default to the device-local AES key to avoid unexplained Keychain prompts. Local keys are non-portable unless exported with explicit tooling. |
