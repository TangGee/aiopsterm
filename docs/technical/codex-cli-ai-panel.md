...
In Codex/Host Agent mode, the project-files drawer starts below the target bar. This keeps the bound host target tab clickable while the file view is open; switching the target still preserves the selected files/AI surface preference for that terminal.
