...
Contracts are in `src/shared/contracts/terminalRecovery.ts`. The preload API exposes `loadTerminalRecovery()` and `saveTerminalRecovery(snapshot)`, implemented by `terminal:recovery:load` and `terminal:recovery:save` in Main. Live sessions are returned only for the requesting owner window and a matching local/SSH kind and SSH host, port and username. The latest verified live cwd takes precedence over the saved directory. They are never recovered solely because a stale disk ID exists.

Main writes `terminal-recovery.json` under Electron userData using ordered temporary-file writes and rename. Temporary files are set to mode 0600 before writing, including when reusing a file left by an interrupted save. The schema is version 1 and allows at most 32 terminal tabs, 128 Ki characters of history per tab and 16 MiB per file. It whitelists metadata and asset references; passwords, private keys, passphrases, arbitrary environment objects and transport-pool aliases are not stored. Visible terminal output may itself contain sensitive text.
...
Checkpointing runs every five seconds, with a one-second debounce for tab/directory/layout metadata and a best-effort pagehide save. Unchanged snapshots are skipped. One save runs at a time and worker reads are sequential. A removed tab is filtered again before saving; disabling recovery during capture clears the checkpoint. Disposal stops pending recovery work. Restoration never replaces a user-opened editor or terminal while disk loading is pending. Each tab launch fails independently, and late launch results from a removed tab or a superseded launch are closed instead of attached. Disk errors surface a notice; missing/corrupt snapshots leave normal startup available. Latest output since the checkpoint may be lost after a crash or reload. This is bounded display recovery, not an output journal or process checkpoint.
