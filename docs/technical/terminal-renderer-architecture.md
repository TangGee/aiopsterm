...
Worker messages must stay structured-clone safe. Renderer settings, theme values, and keyword-highlight config are normalized into plain data before `postMessage()` so Vue/Pinia proxies cannot make workspace terminals fail open and fall back to the legacy xterm renderer. Codex conversations also store terminal hosts, FitAddon instances, and ResizeObserver instances with Vue `markRaw`, preventing reactive proxies around Worker-bearing runtime handles during first mount.
