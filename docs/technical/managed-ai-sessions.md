...
- Session rows use the latest summary/detail as the primary two-line text, with a compact meta line for status, agent, relative time, path when space allows, and a double-click hint. Auto/manual titles are still stored for details, tooltips, copied terminal context, and restore terminal titles, but they are not the primary row label. Internal lifecycle states such as `ended`, `idle`, and `unknown` are used for filtering and reconciliation, but the session library does not present them as user-facing row statuses.
- Selecting a row opens details inside the left AI session panel without changing the active terminal. The shared main work area remains the terminal workspace.
