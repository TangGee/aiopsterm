...
The content API uses the same managed-session store as `listManagedAiSessions()`. A content list call first refreshes local history import so Codex and Claude Code rows usually have a `transcriptPath`, and OpenCode rows can be resolved from `OPENCODE_CONFIG_DIR/opencode.db`.
...
Content editing is intentionally conservative. Sessions in `working` or `needsInput` state are read-only; the user must stop, finish, or handle the active session before changing local transcript content. Every update carries a `sourceRevision` derived from the source file or database stat data. If the file/database changes after the list/get call, saving fails with `MANAGED_AI_CONTENT_REVISION_CONFLICT` and the renderer reloads before retrying. Before a successful write, aiopsterm copies the source JSONL or OpenCode database files into `agent-sessions/content-backups/<source>/<session>/` under the app user data directory and keeps a bounded recent backup set.
