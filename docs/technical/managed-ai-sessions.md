...
- `listManagedAiSessionContent({ source, sessionId, offset, limit, maxContentChars })`
...
Supported first-version stores:

- Codex and Claude Code JSONL transcripts. The runtime parses each JSONL line, recursively extracts text-bearing fields such as `text`, `content`, `message`, `output`, and `arguments`, and skips protocol discriminator fields such as `type`, `role`, ids, timestamps, model metadata, and Codex runtime-only `turn_context.summary = auto` entries. Codex `function_call` argument fields and matching `function_call_output` output fields remain separate timeline records; their `messageType` is normalized to `tool call` or `tool result`, with the tool name carried across by `call_id` when available. Each extracted text field records the line number plus JSON pointer as `recordId`, and save edits exactly that field.
...
- Other managed AI sources fall back to stored event summaries and compact raw event payloads. This gives a browseable session history, but not full transcript editing, until a source-specific local state adapter is added.

Content editing is intentionally conservative. Sessions in `working` or `needsInput` state are read-only; the user must stop, finish, or handle the active session before changing local transcript content. Every update or delete carries a `sourceRevision` derived from the source file or database stat data. If the file/database changes after the list/get call, saving or deleting fails with `MANAGED_AI_CONTENT_REVISION_CONFLICT` and the renderer reloads before retrying. Before a successful write/delete, aiopsterm copies the source JSONL or OpenCode database files into `agent-sessions/content-backups/<source>/<session>/` under the app user data directory and keeps a bounded recent backup set. JSONL deletion removes the selected text field and drops the whole transcript line only when that line no longer has browseable text records; OpenCode deletion removes the selected editable `part` row.
...
The workspace renders content as a single full-record card stream rather than as a raw field index or separate conversation/context filters. The renderer requests the first page with a small limit, then appends later pages from idle/animation-frame work triggered by scroll position, viewport fill, or active search prefetch; it intentionally has no manual `Load more` button. This keeps large transcript switching from blocking the renderer main thread while preserving access to every record. Role labels explain each record in place, while system/developer prompts, metadata, tool/event records, reasoning blocks, file snapshots, unknown fallback records, long records, and truncated records are collapsed by default. Collapsed previews end with an inline expand action that loads the full record before editing. This keeps extracted JSONL fields such as `session_meta` and `payload/base_instructions/text` browseable without forcing the user to choose a classification before reading the session.
...
The renderer hydrates from this store on startup through `src/renderer/src/services/ai/managedAiClient.ts`, which owns `listManagedAiSessions()` bridge lookup and binding. The list call returns the stored snapshot and lets local history import plus git metadata refresh continue asynchronously. Incoming hook events update the in-memory UI immediately and are persisted by the main process. Background import or git refresh completion is reported through managed-AI events, and the renderer reloads the list after those invalidations. `stop` is treated as a user-facing turn-complete notification and moves the session to `needsInput` for review/confirmation even when the agent reports an `idle` lifecycle. `session_end`, terminal close/error events, and explicit terminal panel closes are the paths that mark a bound session ended. After applying a backend snapshot, the renderer reconciles `working` and `needsInput` rows with locally live terminal panels; if a row is bound to a panel/session id that no longer exists locally, the UI marks it ended so stale history does not remain in the Running view.
