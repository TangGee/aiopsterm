...
const clickHouseConnectionInput = (connection: DatabaseConnectionInfo): DatabaseConnectionTestInput =>
...
const clickHouseWhereForFilters = (filters: ReturnType<typeof parseWhereRaw>, knownColumns: string[]) => {
...
const clickHouseOrderByFor = (sort: DatabaseTableQueryInput['sort'] | null | undefined, knownColumns: string[]) => {
  if (!sort) return ''
  const known = new Map(knownColumns.map((column) => [column.toLowerCase(), column]))
...
export const clickHouseMutateTable = async (
  ...
  try {
    ...
    await configuredDatabaseHttpRuntime().refreshConnectionCatalog(connection.id, clickHouseCatalogsForConnection)
    ...
    return {
      ...
      data: {
        ...
        durationMs: Math.max(1, Date.now() - startedAt),
        catalog: configuredDatabaseHttpRuntime().workspaceCatalogFor(input.connectionId)
      }
    }
  } catch (error) {
    return {
