...
type DatabaseMutationDialect = Exclude<DatabaseEngineCode, 'clickhouse' | 'presto'>
...
const isPrestoConnection = (connection: DatabaseConnectionInfo | null | undefined): connection is PrestoDatabaseConnection =>
...
const prestoBaseUrlFrom = (input: Pick<DatabaseConnectionTestInput, 'host' | 'port' | 'url'>) => {
  const rawUrl = trim(input.url)
  if (/^https?:\/\//i.test(rawUrl)) return rawUrl.replace(/\/+$/, '')
  const host = trim(input.host)
...
const prestoEndpointFor = (input: Pick<DatabaseConnectionTestInput, 'host' | 'port' | 'url'>) => prestoBaseUrlFrom(input)

const prestoConnectionInput = (connection: DatabaseConnectionInfo): DatabaseConnectionTestInput => connectionTestInputFromSaved(connection)
...
const prestoErrorCode = (error: unknown, fallback: string) => {
  const code = error && typeof error === 'object' && 'code' in error ? String((error as { code?: unknown }).code || '') : ''
  return code.startsWith('DB_') ? code : fallback
}

const prestoErrorMessage = (error: unknown, fallback: string) => (error instanceof Error ? error.message : String(error || fallback))
...
const prestoQuery = async <T extends Record<string, unknown>>(
  input: DatabaseConnectionTestInput,
  sql: string,
  context: Pick<DatabaseTableQueryInput, 'databaseName' | 'schemaName'> = { databaseName: '', schemaName: '' }
...
const prestoCatalogsForConnection = async (connection: DatabaseConnectionInfo): Promise<DatabaseCatalogInfo[]> => {
...
const prestoTableDdl = async (connection: DatabaseConnectionInfo, input: DatabaseTableDdlInput): Promise<DatabaseTableDdlResult> => {
  try {
    const rows = await prestoRows<Record<string, unknown>>(
      connection,
...
const prestoMutationUnsupported = () => ({
  ok: false as const,
