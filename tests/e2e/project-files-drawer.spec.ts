...
test('project files switches the persistent AI panel content surface', async () => {
  ...
  try {
    ...
    const editorLines = projectEditor.locator('.view-lines')
    await editorLines.click()
    await page.keyboard.press('ControlOrMeta+A')
    await editorLines.click({ button: 'right', position: { x: 24, y: 10 } })
    const editorContextMenu = page.getByTestId('text-editor-context-menu')
    await expect(editorContextMenu).toBeVisible()
    await expect(editorContextMenu.getByRole('menuitem', { name: /剪切/ })).toBeEnabled()
    await expect(editorContextMenu.getByRole('menuitem', { name: /复制/ })).toBeEnabled()
    const editorMenuBox = await editorContextMenu.boundingBox()
    const editorMenuViewport = page.viewportSize()!
    expect(editorMenuBox!.x).toBeGreaterThanOrEqual(8)
    expect(editorMenuBox!.y).toBeGreaterThanOrEqual(8)
    expect(editorMenuBox!.x + editorMenuBox!.width).toBeLessThanOrEqual(editorMenuViewport.width - 8)
    expect(editorMenuBox!.y + editorMenuBox!.height).toBeLessThanOrEqual(editorMenuViewport.height - 8)
    await editorContextMenu.getByRole('menuitem', { name: /剪切/ }).click()
    await expect(editorLines).not.toContainText('export const ready = true')
    await editorLines.click({ button: 'right' })
    await editorContextMenu.getByRole('menuitem', { name: /撤销/ }).click()
    await expect(editorLines).toContainText('export const ready = true')
