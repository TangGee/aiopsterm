...
test('project files uses a contextual drawer inside the persistent AI panel', async () => {
  ...
  try {
    ...
    await expect(projectFileContextMenu.getByRole('menuitem', { name: 'Rename' })).toBeVisible()
    await expect(projectFileContextMenu.getByRole('menuitem', { name: 'Delete' })).toBeVisible()
    await expect(projectFileContextMenu.getByRole('menuitem', { name: 'Copy relative path' })).toBeVisible()
    await expect(projectFileContextMenu.getByRole('menuitem', { name: 'Copy absolute path' })).toBeVisible()
    await projectFileContextMenu.getByRole('menuitem', { name: 'Copy relative path' }).click()
    await expect(drawer.locator('.project-files-notice')).toHaveText('Relative path copied')
    await expect(drawer).toBeVisible()
    ...
    await projectFileContextMenu.getByRole('menuitem', { name: 'Rename' }).click()
    ...
    await projectFileContextMenu.getByRole('menuitem', { name: 'Delete' }).click()
    mutationDialog = page.locator('.project-files-dialog')
