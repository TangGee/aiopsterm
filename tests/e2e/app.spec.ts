...
test('aiopsterm primary desktop flows', async () => {
  ...
  try {
    ...
    await expect(page.locator('.terminal-output-mirror').filter({ hasText: 'aiopsterm ssh ops@10.24.8.12:22' })).toHaveCount(1)
    ...
    await expect(page.locator('.terminal-output-mirror').filter({ hasText: 'aiopsterm ssh ops@10.72.0.5:2205' })).toHaveCount(1)
    await page.locator('.workspace-tabs button').filter({ hasText: '堡垒机资源' }).click()
    ...
    await expect(page.locator('.terminal-output-mirror').filter({ hasText: 'aiopsterm ssh ops@10.66.0.8:2200' })).toHaveCount(1)
    ...
    await expect(page.locator('.terminal-pane.active .terminal-output-mirror')).toContainText('aiopsterm ssh ops@10.66.0.8:2200')
