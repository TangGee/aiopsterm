...
test('aiopsterm primary desktop flows', async () => {
  ...
  try {
    ...
    await expect(page.locator('.terminal-output-mirror').filter({ hasText: 'df -h' })).toHaveCount(1)
    await expect(page.locator('.terminal-output-mirror').filter({ hasText: 'free -m' })).toHaveCount(1)
    await expect(page.locator('.terminal-output-mirror').filter({ hasText: 'uptime' })).toHaveCount(1)
