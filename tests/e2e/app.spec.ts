...
test('aiopsterm primary desktop flows', async () => {
  ...
  try {
    ...
    await expect(page.locator('.terminal-output-mirror').filter({ hasText: '[mock ssh] workspace-e2e' })).toHaveCount(1)
    await page.locator('.workspace-tabs button').filter({ hasText: '堡垒机资源' }).click()
    ...
    await expect(page.locator('.message.user').filter({ hasText: '请分析这个 Kubernetes 输出' })).toBeVisible()
    ...
    await page.locator('.k8s-search-header .k8s-search').getByTitle('清除搜索').click()
