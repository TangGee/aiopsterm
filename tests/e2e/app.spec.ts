...
test('aiopsterm primary desktop flows', async () => {
  ...
  try {
    ...
    await expect(page.locator('.terminal-output-mirror').filter({ hasText: '[mock ssh] prod-bastion' })).toHaveCount(1)
    ...
    await expect(page.locator('.terminal-output-mirror').filter({ hasText: '[mock ssh] e2e-host' })).toHaveCount(1)
    ...
    await page.locator('.key-form-panel label').filter({ hasText: '公钥' }).locator('textarea').fill('ssh-ed25519 AAAA e2e')
    ...
    await page.locator('.file-more-menu button').filter({ hasText: '移动' }).click()
    await expect(page.getByText('移动到')).toBeVisible()
    ...
    await expect(page.locator('.terminal-output-mirror').filter({ hasText: '[snippet] 磁盘巡检' })).toHaveCount(1)
    ...
    await page.locator('.k8s-search').first().locator('input').fill('')
    ...
    await expect(page.getByText('[mock kubectl] kubectl get pods -A')).toBeVisible()
    ...
    await page.locator('.k8s-resource-table tbody tr').filter({ hasText: 'billing-worker-7f9d6f9dd9-rx8mm' }).getByTitle('发送到终端').click()
    await expect(page.getByText('[mock kubectl] kubectl describe pod billing-worker-7f9d6f9dd9-rx8mm -n ops')).toBeVisible()
    ...
    await expect(dbConnectionModal).toContainText('PostgreSQL 16 mock')
    ...
    await expect(page.locator('.db-workspace-tab').filter({ hasText: 'orders-postgres SQL' })).toBeVisible()
    ...
    await page.locator('.db-result-tabs button').filter({ hasText: 'Overview' }).click()
    await expect(page.locator('.db-sql-overview')).toContainText('Execution History')
    await expect(page.locator('.db-sql-overview')).toContainText('1 executions')
    ...
    await expect(page.locator('.db-result-table').filter({ hasText: 'mock query ok' })).toBeVisible()
    ...
    await page.locator('.user-modal-card footer .primary').click()
    await expect(page.locator('.user-info-notice')).toContainText('密码重置为本地占位')

    await page.locator('.user-avatar.large').click()
    await expect(page.locator('.avatar-settings-modal')).toBeVisible()
    await page.locator('.avatar-settings-modal input').first().fill('EU')
    await page.locator('.avatar-settings-modal footer .primary').click()
    await expect(page.locator('.user-avatar.large')).toContainText('EU')

    await page.locator('.user-info-footer .danger').click()
    await expect(page.locator('.user-login-card')).toContainText('请先登录')
    await page.locator('.user-login-card .primary').click()
    await expect(page.locator('.user-info-card')).toContainText('E2E Operator')
    ...
    await expect(page.getByText('K8s 发布失败')).toBeVisible()
    await expect(page.getByText('生产巡检')).not.toBeVisible()
    ...
    await expect(page.getByTestId('ai-file-upload-button')).toHaveAttribute('title', '上传文件暂未启用')
    await page.getByTestId('ai-file-upload-button').click()
    await expect(page.locator('.input-placeholder-notice')).toContainText('文件上传为本地占位')
    await expect(page.getByTestId('ai-voice-button')).toBeVisible()
    await expect(page.getByTestId('ai-voice-button')).toHaveAttribute('title', '语音输入暂未启用')
    await page.getByTestId('ai-voice-button').click()
    await expect(page.locator('.input-placeholder-notice')).toContainText('语音输入为本地占位')
    ...
    await expect(page.getByText('当前为本地 mock 响应')).toBeVisible()
...
test('terminal tab operations and visual baseline', async () => {
  ...
  try {
    ...
    await expect(page.locator('.terminal-output-mirror').first()).toContainText('[mock] df -h')
    ...
    await expect(page.locator('.terminal-output-mirror').first()).not.toContainText('[mock] df -h')

    await page.locator('.command-line input').first().fill('journalctl')
    await page.locator('.command-line input').first().press('Enter')
    await page.locator('.terminal-search input').fill('journalctl')
    ...
    await page.locator('.terminal-search-overlay input').fill('journalctl')
    ...
    await expect(page.locator('.terminal-output-mirror').first()).toContainText('[mock broadcast] uptime')
