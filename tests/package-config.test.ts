...
describe('package configuration audit', () => {
  ...
  it('packages self-owned documentation outside app.asar for runtime help pages', () => {
    const builderConfig = readFileSync('electron-builder.yml', 'utf8')
    const mainSource = readFileSync('src/main/index.ts', 'utf8')
  ...
  it('keeps the local macOS one-click build entrypoint available', async () => {
    ...
    expect(help.stdout).toContain('--notary-profile NAME')
    expect(help.stdout).toContain('--release')
    expect(help.stdout).toContain('--require-release-signing')
    ...
    expect(script).toContain('Developer ID Application identity detected')
    expect(script).toContain('notarytool history --keychain-profile')
    expect(script).toContain('CSC_IDENTITY_AUTO_DISCOVERY=false')
    expect(script).toContain('AIOPSTERM_MAC_FORCE_ADHOC_SIGN=1')
    expect(script).toContain('prepare_build_electron_runtime')
    expect(script).toContain('xattr -dr com.apple.quarantine')
    expect(script).toContain('codesign --force --deep --sign - "${electron_app}"')
    expect(script).toContain('ELECTRON_RUN_AS_NODE=1 "${electron_executable}"')
