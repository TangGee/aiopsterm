...
describe('workspace store', () => {
  ...
  it('moves completed managed AI turns to pending validation and clears attention on terminal exit', () => {
    ...
    expect(store.markManagedAiSessionHandled('claude-code', 'claude-session-1')).toBe(true)
  ...
  it('hibernates managed AI sessions by killing the owning terminal and marking metadata', async () => {
    ...
    vi.mocked(window.aiops.replyManagedAiSession).mockResolvedValueOnce({ ok: false } as any)
    store.markManagedAiSessionHandled('codex', 'codex-hibernate-1')
    vi.mocked(window.aiops.killTerminal).mockClear()
  ...
  it('aborts an active Cline Agent command even when its command card is not streaming', async () => {
    ...
    await expect(store.cancelStreamingAiChatResponse()).resolves.toBe(true)
    expect(window.aiops.abortClineAgentTask).toHaveBeenCalledWith({
      taskId: 'cline-task-1',
      turnId: 'cline-turn-1',
      reason: 'The operator stopped the Classic Agent turn.'
    })
    expect(window.aiops.cancelAiChatResponse).not.toHaveBeenCalled()
    expect(store.chatMessages[0]).toMatchObject({
      state: 'done',
      commandExecutionStatus: 'failed',
      commandExecutionMessage: 'Cline Agent 命令已停止。',
  ...
  it('does not continue the legacy renderer agent loop after Cline migration', async () => {
    ...
    expect(result).toMatchObject({ status: 'unavailable', reason: expect.stringContaining('Cline runtime') })
  ...
  it('does not resend long command output from the renderer after Cline migration', async () => {
    const store = useWorkspaceStore()
    ...
    expect(store.chatMessages.find((message) => message.say === 'command_output')).toBeUndefined()
    expect(window.aiops.generateAiChatResponse).not.toHaveBeenCalled()
  })

  it('does not resend unfiltered command output from the renderer after Cline migration', async () => {
    const store = useWorkspaceStore()
  ...
  it('does not auto-run Agent command cards from the renderer when the global legacy preference is enabled', async () => {
    const store = useWorkspaceStore()
    ...
    expect(store.chatMessages.find((message) => message.id === 'aichat-request-test-1-assistant')?.executedCommand).toBeUndefined()
  })

  it('keeps the legacy per-session Agent auto-run toggle disabled after Cline migration', async () => {
    const store = useWorkspaceStore()
    await store.restoreConversation('conv-1')
  ...
  it('reports the legacy renderer Agent loop as unavailable without calling the provider', async () => {
    ...
    expect(result.status).toBe('unavailable')
    expect(store.chatMessages.some((message) => message.say === 'command_output')).toBe(false)
    expect(store.chatMessages.find((message) => message.id === 'command-ask')).toMatchObject({ commandExecutionStatus: 'running' })
    expect(window.aiops.generateAiChatResponse).not.toHaveBeenCalled()
  })
  ...
  it('fails closed when the quick command backend returns malformed snapshots during hydration', async () => {
    const store = useWorkspaceStore()
    vi.mocked(window.aiops.getQuickCommands).mockResolvedValueOnce({
      groups: [{ id: 9, uuid: 'detached-group', group_name: 'Detached' }],
      snippets: [
        {
          id: 9,
  ...
  it('hydrates and migrates External reference-style AI preference settings', async () => {
    ...
    vi.mocked(window.aiops.getConfig).mockResolvedValueOnce({
      ...
      aiPreferences: {
        ...
        managedAiAutoNamingEnabled: true,
        autoApproval: true,
  ...
  it('manages External reference-style quick command scripts and macro snippets', async () => {
    ...
    if (missingPlannerDecision?.status === 'unavailable') {
      expect(missingPlannerDecision.reason).toBe('快捷命令执行计划服务不可用')
    }
    expect(window.aiops.writeTerminal).not.toHaveBeenCalled()
    ...
    if (failedPlannerDecision?.status === 'unavailable') {
      expect(failedPlannerDecision.reason).toBe('planner offline')
    }
    expect(store.topNotice).toBe('planner offline')
    expect(window.aiops.writeTerminal).not.toHaveBeenCalled()
    ...
    vi.mocked(window.aiops.writeTerminal).mockResolvedValueOnce({
      ...
      errorMessage: 'terminal closed'
    })
    const failedDelayedDecision = await store.runQuickCommand(delayedFailureSnippet!.id, true)
    expect(failedDelayedDecision?.status).toBe('unavailable')
    expect(window.aiops.writeTerminal).toHaveBeenCalledTimes(1)
  ...
  it('does not mutate quick commands when required backend operations are unavailable, malformed, or fail', async () => {
    const store = useWorkspaceStore()
    await store.refreshQuickCommands()
    ...
    try {
      ...
      vi.mocked(window.aiops.getQuickCommands!).mockResolvedValueOnce({
        ...
        snippets: [
          ...
          {
            id: 999,
            uuid: 'detached-refresh-snippet',
            snippet_name: 'Detached Refresh',
            snippet_content: 'echo detached',
            group_uuid: 'missing-refresh-group'
          }
        ]
      } as any)
      await expect(store.refreshQuickCommands()).resolves.toBe(false)
  ...
  it('manages External reference-style Kubernetes contexts, clusters, terminals, and bastion sync', async () => {
    ...
    await expect(store.executeK8sTerminalAiCommand('   ', newTerminal.id)).resolves.toBe(false)
    ...
    expect(store.k8sActiveTerminal?.aiCommandTabId).toBeNull()
...
${JSON.stringify(externalSecurityConfig, null, 2)}`)
    ...
    expect(store.settingsDocumentationOpen).toBe(true)
    expect(store.settingsDocumentationContent).toContain('# aiopsterm Docs')
    vi.mocked(window.aiops.openSettingsDocumentation).mockClear()
    await store.openSettingsDocumentationLink('usage/index.md')
    ...
    expect(store.settingsDocumentationTitle).toBe('Usage Docs')
    vi.mocked(window.aiops.openSettingsDocumentation).mockClear()
    store.setActiveSettingsSection('terminal')
    await store.openSettingsPageDocumentation('terminal')
    expect(store.activeSettingsSection).toBe('terminal')
    expect(window.aiops.openSettingsDocumentation).toHaveBeenCalledWith({ page: 'terminal', locale: 'zh-CN' })
    expect(store.settingsDocumentationOpen).toBe(true)
    store.closeSettingsDocumentation()
    expect(store.settingsDocumentationOpen).toBe(false)
  ...
  it('does not fabricate Settings external action success when the preload bridge is unavailable or fails', async () => {
    ...
    try {
      ...
      vi.mocked(window.aiops.openSettingsDocumentation).mockResolvedValueOnce({ path: '   ', title: 'Docs', content: '# Docs' })
      store.setActiveSettingsSection('docs')
      await Promise.resolve()
      await Promise.resolve()
      expect(store.settingsNotice).toBe('文档入口打开失败')
      vi.mocked(window.aiops.openSettingsDocumentation).mockResolvedValueOnce({ path: '/tmp/docs/index.md', title: '   ', content: '# Docs' })
      store.setActiveSettingsSection('docs')
      await Promise.resolve()
      await Promise.resolve()
      expect(store.settingsNotice).toBe('文档入口打开失败')
  ...
  it('fails closed on malformed Skill backend results instead of mutating settings state', async () => {
    const store = useWorkspaceStore()
    await store.refreshSkillsFromBridge()
    ...
    const originalAiops = {
      getSkills: window.aiops.getSkills,
      getSkillsUserPath: window.aiops.getSkillsUserPath,
      reloadSkills: window.aiops.reloadSkills,
      readSkillContent: window.aiops.readSkillContent,
      createSkill: window.aiops.createSkill,
      updateSkill: window.aiops.updateSkill,
      ...
      exportSkillZip: window.aiops.exportSkillZip,
      showOpenDialog: window.aiops.showOpenDialog,
      onSkillsUpdate: window.aiops.onSkillsUpdate
    }
    ...
    try {
      ...
      await expect(store.refreshSkillsFromBridge()).resolves.toBe(false)
      expect(store.settingsNotice).toBe(malformedSkillsBackendResultMessage)
      expect(JSON.stringify(store.settingsSkills)).toBe(originalSkills)

      vi.mocked(window.aiops.getSkillsUserPath!).mockResolvedValueOnce(42 as any)
      vi.mocked(window.aiops.getSkills).mockResolvedValueOnce([
        {
          name: 'bad-path-skill',
      ...
      vi.mocked(window.aiops.reloadSkills).mockResolvedValueOnce([{ name: 'bad-reload' }] as any)
      await expect(store.reloadSkills()).resolves.toBe(false)
      expect(store.settingsNotice).toBe(malformedSkillsBackendResultMessage)
      expect(JSON.stringify(store.settingsSkills)).toBe(originalSkills)
      ...
      {
        setActivePinia(createPinia())
        const listenerStore = useWorkspaceStore()
        let skillsUpdateListener: ((skills: any[]) => void) | null = null
        vi.mocked(window.aiops.onSkillsUpdate).mockImplementationOnce((listener) => {
          skillsUpdateListener = listener
          return () => undefined
        })
        await expect(listenerStore.refreshSkillsFromBridge()).resolves.toBe(true)
        const listenerOriginalSkills = JSON.stringify(listenerStore.settingsSkills)
        expect(skillsUpdateListener).toBeTruthy()
        skillsUpdateListener!([{ name: 'bad-listener-skill' }])
        expect(listenerStore.settingsNotice).toBe(malformedSkillsBackendResultMessage)
        expect(JSON.stringify(listenerStore.settingsSkills)).toBe(listenerOriginalSkills)
      }

      vi.mocked(window.aiops.readSkillContent).mockResolvedValueOnce({
        metadata: { name: 'other-skill', description: 'Wrong skill' },
      ...
      await store.openSkillModal('create')
      store.skillModal.name = 'bad-created-skill'
      store.skillModal.description = 'bad created description'
      store.skillModal.content = 'bad created content'
      vi.mocked(window.aiops.createSkill).mockResolvedValueOnce({
        name: 'bad-created-skill',
        description: 'bad created description',
        enabled: true,
        editable: true,
        content: ''
      } as any)
      await expect(store.saveSkillModal()).resolves.toBe(false)
      expect(store.settingsNotice).toBe(malformedSkillsBackendResultMessage)
      expect(store.settingsSkills.some((skill) => skill.name === 'bad-created-skill')).toBe(false)
      expect(JSON.stringify(store.settingsSkills)).toBe(originalSkills)
      ...
      await expect(store.saveSkillModal()).resolves.toBe(false)
      expect(store.settingsNotice).toBe(malformedSkillsBackendResultMessage)
      expect(store.settingsSkills.some((skill) => skill.name === 'detached-created-skill')).toBe(false)
      expect(JSON.stringify(store.settingsSkills)).toBe(originalSkills)
      ...
      vi.mocked(window.aiops.showOpenDialog).mockResolvedValueOnce({ canceled: false, filePaths: ['/tmp/malformed-skill.zip'] })
      vi.mocked(window.aiops.importSkillZip).mockResolvedValueOnce({ success: true } as any)
      await expect(store.importSkillZip()).resolves.toBe(false)
      expect(store.settingsNotice).toBe(malformedSkillsBackendResultMessage)
      expect(JSON.stringify(store.settingsSkills)).toBe(originalSkills)
      ...
      await expect(store.importSkillZip()).resolves.toBe(false)
      expect(store.settingsNotice).toBe(malformedSkillsBackendResultMessage)
      expect(store.settingsSkills.some((skill) => skill.name === 'detached-import')).toBe(false)
      expect(JSON.stringify(store.settingsSkills)).toBe(originalSkills)
      ...
      store.chatMessages.push({
        id: 'assistant-skill-malformed',
        role: 'assistant',
        text: 'Turn this malformed create into a skill',
        state: 'done'
      })
      vi.mocked(window.aiops.createSkill).mockResolvedValueOnce({
        name: 'turn-this-malformed-skill',
        description: 'Summarized from AI message assistant-skill-malformed',
        enabled: true,
        editable: true,
        content: ''
      } as any)
      await expect(store.summarizeMessageToSkill('assistant-skill-malformed')).resolves.toBeNull()
      expect(store.settingsNotice).toBe(malformedSkillsBackendResultMessage)
      expect(store.settingsSkills.some((skill) => skill.name.includes('turn-this-malformed'))).toBe(false)
      expect(JSON.stringify(store.settingsSkills)).toBe(originalSkills)
    } finally {
      Object.assign(window.aiops, originalAiops)
    }
  ...
  it('does not fabricate MCP mutation success when backend mutation snapshots do not match the request', async () => {
    const store = useWorkspaceStore()
    await store.refreshMcpServersFromBridge()
    const originalServers = JSON.stringify(store.mcpServers)
    ...
    vi.mocked(window.aiops.toggleMcpServer).mockResolvedValueOnce({ ok: true, data: staleData })
    await expect(store.toggleMcpServerDisabled('filesystem')).resolves.toBe(false)
    expect(store.settingsNotice).toBe('MCP filesystem 状态更新结果不匹配')
    expect(JSON.stringify(store.mcpServers)).toBe(originalServers)

    vi.mocked(window.aiops.setMcpToolState).mockResolvedValueOnce({ ok: true, data: staleData })
    await expect(store.toggleMcpTool('filesystem', 'read_file')).resolves.toBe(false)
    expect(store.settingsNotice).toBe('read_file 状态更新结果不匹配')
    expect(JSON.stringify(store.mcpServers)).toBe(originalServers)

    vi.mocked(window.aiops.deleteMcpServer).mockResolvedValueOnce({ ok: true, data: staleData })
    await expect(store.deleteMcpServer('ops-inventory')).resolves.toBe(false)
    expect(store.settingsNotice).toBe('ops-inventory 删除结果不匹配')
    expect(JSON.stringify(store.mcpServers)).toBe(originalServers)

    vi.mocked(window.aiops.setMcpToolAutoApprove).mockResolvedValueOnce({ ok: true, data: staleData })
    await expect(store.toggleMcpToolAutoApprove('filesystem', 'read_file')).resolves.toBe(false)
    expect(store.settingsNotice).toBe('MCP Auto Approve 更新结果不匹配')
    expect(store.mcpConfigEditorError).toBe('Auto Approve failed: MCP Auto Approve 更新结果不匹配')
    expect(JSON.stringify(store.mcpServers)).toBe(originalServers)
  })

  it('does not fabricate MCP mutation success when refreshed backend state does not match the request', async () => {
    const withStaleMcpRefresh = async () => {
      ;(globalThis as any).__resetMcpStoreMock?.()
      setActivePinia(createPinia())
      const store = useWorkspaceStore()
      await store.refreshMcpServersFromBridge()
      ...
      return { store, originalServers, staleServers }
    }

    let stale = await withStaleMcpRefresh()
    await expect(stale.store.toggleMcpServerDisabled('filesystem')).resolves.toBe(false)
    expect(stale.store.settingsNotice).toBe('MCP filesystem 状态更新后刷新结果不匹配')
    expect(JSON.stringify(stale.store.mcpServers)).toBe(stale.originalServers)

    stale = await withStaleMcpRefresh()
    await expect(stale.store.toggleMcpTool('filesystem', 'read_file')).resolves.toBe(false)
    expect(stale.store.settingsNotice).toBe('read_file 状态更新后刷新结果不匹配')
    expect(JSON.stringify(stale.store.mcpServers)).toBe(stale.originalServers)

    stale = await withStaleMcpRefresh()
    await expect(stale.store.deleteMcpServer('ops-inventory')).resolves.toBe(false)
    expect(stale.store.settingsNotice).toBe('ops-inventory 删除后刷新结果不匹配')
    expect(JSON.stringify(stale.store.mcpServers)).toBe(stale.originalServers)

    stale = await withStaleMcpRefresh()
    await expect(stale.store.toggleMcpToolAutoApprove('filesystem', 'read_file')).resolves.toBe(false)
    expect(stale.store.settingsNotice).toBe('MCP Auto Approve 更新后刷新结果不匹配')
    expect(stale.store.mcpConfigEditorError).toBe('Auto Approve failed: MCP Auto Approve 更新后刷新结果不匹配')
    expect(JSON.stringify(stale.store.mcpServers)).toBe(stale.originalServers)
  })
  ...
  it('runs MCP tools and reads resources only through preload operation bridges', async () => {
    ...
    const toolKey = store.getMcpToolOperationKey('filesystem', 'read_file')
    store.updateMcpToolArgumentDraft('filesystem', 'read_file', '{"path":"/tmp/readme.md","encoding":"utf-8"}')
    vi.mocked(window.aiops.callMcpTool).mockClear()
    await expect(store.runMcpTool('filesystem', 'read_file')).resolves.toBe(true)
    expect(window.aiops.callMcpTool).toHaveBeenCalledWith('filesystem', 'read_file', {
      path: '/tmp/readme.md',
      encoding: 'utf-8'
    })
    ...
    expect(store.mcpOperationResults[toolKey].output).toContain('MCP tool filesystem:read_file executed.')
    expect(store.settingsNotice).toBe('read_file 调用完成')

    const resourceKey = store.getMcpResourceOperationKey('filesystem', 'file:///workspace')
    ...
    expect(store.mcpOperationResults[resourceKey].output).toContain('MCP resource file:///workspace')
    expect(store.settingsNotice).toBe('workspace-root 读取完成')
  })

  it('does not fabricate MCP operation results when arguments, bridges, or envelopes are invalid', async () => {
    const store = useWorkspaceStore()
    await store.refreshMcpServersFromBridge()
    const toolKey = store.getMcpToolOperationKey('filesystem', 'read_file')
    const resourceKey = store.getMcpResourceOperationKey('filesystem', 'file:///workspace')
    const originalAiops = {
      callMcpTool: window.aiops.callMcpTool,
      readMcpResource: window.aiops.readMcpResource
    }
