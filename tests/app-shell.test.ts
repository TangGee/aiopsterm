...
describe('AppShell', () => {
  ...
  it('applies persisted background and watermark settings at the app shell level', async () => {
    ...
    expect(styles).toContain('min-inline-size: 112px;')
  ...
  it('matches External reference-style terminal context menu, search overlay, suggestions, and global command bar', async () => {
    ...
    window.addEventListener('keydown', bubbledEnter)
    await wrapper.find('.terminal-search-overlay input').trigger('keydown', { key: 'Enter' })
    expect(bubbledEnter).not.toHaveBeenCalled()
    expect(wrapper.find('.terminal-search-overlay').text()).toContain('2/2')
    await wrapper.find('.terminal-search-overlay input').trigger('keydown', { key: 'Enter' })
    expect(wrapper.find('.terminal-search-overlay').text()).toContain('1/2')
    window.removeEventListener('keydown', bubbledEnter)
    await wrapper.find('.terminal-search-overlay button[title="下一个"]').trigger('click')
    expect(searchAddon.findNext).toHaveBeenCalledWith('LOOPNEEDLE', { caseSensitive: false })
    await wrapper.find('.terminal-search-overlay button[title="上一个"]').trigger('click')
    expect(searchAddon.findPrevious).toHaveBeenCalledWith('LOOPNEEDLE', { caseSensitive: false })
  ...
  it('keeps readline control keys passthrough while handling terminal app shortcuts', async () => {
    ...
    const searchEvent = new KeyboardEvent('keydown', { key: 'f', ctrlKey: true, altKey: true, bubbles: true, cancelable: true })
    ...
    const searchPreviousEvent = new KeyboardEvent('keydown', { key: 'h', ctrlKey: true, altKey: true, bubbles: true, cancelable: true })
    ...
    const searchClearEvent = new KeyboardEvent('keydown', { key: 'j', ctrlKey: true, altKey: true, bubbles: true, cancelable: true })
    ...
    const terminalHost = wrapper.find('.xterm-host').element
    terminalHost.addEventListener('keydown', (event) => event.stopPropagation(), { once: true })
    const capturedSearchEvent = new KeyboardEvent('keydown', { key: 'f', ctrlKey: true, altKey: true, bubbles: true, cancelable: true })
    terminalHost.dispatchEvent(capturedSearchEvent)
    await wrapper.vm.$nextTick()
    expect(capturedSearchEvent.defaultPrevented).toBe(true)
    expect(wrapper.find('.terminal-search-overlay').exists()).toBe(true)
    wrapper.find('.terminal-search-overlay input').element.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true, cancelable: true }))
    await wrapper.vm.$nextTick()
  ...
  it('matches External reference-style Files workspace modes, sessions, and transfer panel', async () => {
    ...
    try {
      ...
      const defaultBrowser = wrapper.findComponent(FileBrowser)
      const defaultListCallsBeforeSearch = vi.mocked(window.aiops.listFiles).mock.calls.length
      const searchFocusSpy = vi.spyOn(HTMLInputElement.prototype, 'focus')
      await defaultBrowser.find('button[title="搜索当前目录"]').trigger('click')
      expect(searchFocusSpy).toHaveBeenCalled()
      searchFocusSpy.mockRestore()
      expect(defaultBrowser.find('.file-browser-search').exists()).toBe(true)
      await defaultBrowser.find('.file-browser-search input').setValue('RELEASE')
      expect(defaultBrowser.findAll('tbody tr')).toHaveLength(1)
      expect(defaultBrowser.find('tbody tr').text()).toContain('release-note.md')
      expect(vi.mocked(window.aiops.listFiles).mock.calls.length).toBe(defaultListCallsBeforeSearch)
      await defaultBrowser.find('button[title="刷新"]').trigger('click')
      await flushPromises()
      expect((defaultBrowser.find('.file-browser-search input').element as HTMLInputElement).value).toBe('RELEASE')
      await defaultBrowser.find('.file-path-input').setValue('/tmp/local-picked')
      await defaultBrowser.find('.file-path-input').trigger('keydown', { key: 'Enter' })
      await flushPromises()
      expect(defaultBrowser.find('.file-browser-search').exists()).toBe(false)
      ...
      const listCallsBeforeSearch = vi.mocked(window.aiops.listFiles).mock.calls.length
      await leftBrowser.find('button[title="搜索当前目录"]').trigger('click')
      expect(leftBrowser.find('.file-browser-search').exists()).toBe(true)
      expect(rightBrowser.find('.file-browser-search').exists()).toBe(false)
      await leftBrowser.find('.file-browser-search input').setValue('RELEASE')
      expect(leftBrowser.findAll('tbody tr')).toHaveLength(1)
      expect(leftBrowser.find('tbody tr').text()).toContain('release-note.md')
      expect(rightBrowser.text()).toContain('boot')
      expect(vi.mocked(window.aiops.listFiles).mock.calls.length).toBe(listCallsBeforeSearch)
      await leftBrowser.find('.file-browser-search button[title="清空搜索"]').trigger('click')
      expect(leftBrowser.findAll('tbody tr').length).toBeGreaterThan(1)
      await leftBrowser.find('.file-browser-search input').setValue('missing-file')
      expect(leftBrowser.text()).toContain('未找到匹配文件')
      await leftBrowser.find('.file-browser-search input').trigger('keydown', { key: 'Escape' })
      expect(leftBrowser.find('.file-browser-search').exists()).toBe(false)
      await rightBrowser.find('button[title="搜索当前目录"]').trigger('click')
      await rightBrowser.find('.file-browser-search input').setValue('BOOT')
      expect(rightBrowser.findAll('tbody tr')).toHaveLength(1)
      expect(rightBrowser.find('tbody tr').text()).toContain('boot')
      expect(leftBrowser.findAll('tbody tr').length).toBeGreaterThan(1)
      await rightBrowser.find('button[title="搜索当前目录"]').trigger('click')
      expect(rightBrowser.find('.file-browser-search').exists()).toBe(false)
  ...
  it('matches External reference-style remaining settings pages for extensions, nested AI agent pages, shortcuts, privacy, devices, billing, and about', async () => {
    ...
    await workspace.findAll('.diagnostics-card .settings-button').find((button) => button.text().includes('Prepare Report and Open Issue'))!.trigger('click')
    await flushPromises()
    expect(window.aiops.submitSettingsFeedbackReport).toHaveBeenCalled()
    expect(window.aiops.openOfficialExternalLink).toHaveBeenCalledWith('issues')
  })
})
