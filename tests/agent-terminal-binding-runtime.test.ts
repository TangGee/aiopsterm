...
describe('agent integration adapters', () => {
  it('normalizes supported agent aliases through one registry', async () => {
    ...
    expect(normalizeAgentIntegrationSource('kimi')).toBe('kimi-code')
    expect(normalizeAgentIntegrationSource('dsh')).toBe('deepseek-harness')
    ...
    expect(listAgentIntegrationAdapters().map((adapter) => adapter.id)).toEqual(
      expect.arrayContaining(['codex', 'claude-code', 'cursor', 'gemini', 'opencode', 'antigravity', 'hermes-agent', 'kimi-code', 'deepseek-harness'])
    )
  })
})
