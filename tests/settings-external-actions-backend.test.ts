...
describe('settings external action backend boundary', () => {
  ...
  it('opens whitelisted settings page Markdown for the requested locale', async () => {
    ...
    try {
      ...
      const sessionParsersZhPath = join(root, 'docs', 'usage', 'settings', 'zh-CN', 'agent-session-parsers.md')
      const sessionParsersEnPath = join(root, 'docs', 'usage', 'settings', 'en-US', 'agent-session-parsers.md')
      ...
      await writeFile(sessionParsersZhPath, '# Agent 会话解析规则\n', 'utf-8')
      await writeFile(sessionParsersEnPath, '# Agent Session Parser Rules\n', 'utf-8')
      ...
      const zhSessionParsersResult = await backend.openSettingsDocumentation(makeRuntime(root, opened), { page: 'agentSessionParsers', locale: 'zh-CN' })
      const enSessionParsersResult = await backend.openSettingsDocumentation(makeRuntime(root, opened), { page: 'agentSessionParsers', locale: 'en-US' })
      ...
      expect(zhSessionParsersResult).toEqual({ path: sessionParsersZhPath, title: 'Agent 会话解析规则', content: '# Agent 会话解析规则\n' })
      expect(enSessionParsersResult).toEqual({ path: sessionParsersEnPath, title: 'Agent Session Parser Rules', content: '# Agent Session Parser Rules\n' })
